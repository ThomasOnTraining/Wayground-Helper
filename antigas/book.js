(function() {
    ...
    function createGUI() {
        ...
        const styles = `
            ...
            #qia-panel.qia-minimized {
                width: 50px; height: 50px; border-radius: 50%; opacity: 1; pointer-events: auto;
            }
            ...
            #qia-restore-btn-circle {
                display: none; width: 100%; height: 100%; align-items: center;
                justify-content: center; font-size: 28px; cursor: move; user-select: none;
            }
        ...
        const panelHTML = `<div id="qia-header"><h3>🧠 Assistente IA (Groq)</h3><div id="qia-controls"><button id="qia-minimize-btn" title="Minimizar">➖</button></div></div><div id="qia-content"><p>Aguardando o início do jogo...</p></div><div id="qia-restore-btn-circle">🧠</div>`;
    ...
    // --- PARTE 3: LÓGICA DA IA (Sem alterações) ---
    ...
    async function performProxyRequest(prompt, responseDiv, button, dataset) {
        try {
            ...
            if (!response.ok) {
                // Tenta ler a resposta de erro do proxy
    ...
    // --- FUNÇÕES DE AUTO-PREENCHIMENTO (Sem alterações) ---
