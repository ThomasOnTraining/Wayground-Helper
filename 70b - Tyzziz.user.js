...
// @version      13.28
// @description  [FIX] Faz o Piloto Automático (🚀) também atualizar a interface do Foco (🎯) (v13.28).
...
(function() {
    ...
    // --- PARTE 2: GUI (v13.27 - Sem alterações) ---
    ...
    function createGUI() {
        ...
        restoreBtn.addEventListener('click', () => {
            if (!wasDragging) {
                const vw = window.innerWidth; const vh = window.innerHeight; const fullPanelWidth = 380; const minPanelHeight = 100; const padding = 20;
        ...
        GM_xmlhttpRequest({
            ...
            onload: function(response) {
                if (response.status !== 200) {
                    try { const errorData = JSON.parse(response.responseText); responseDiv.textContent = `Erro do Proxy: ${errorData?.error || 'Erro desconhecido'}`;
                    } catch (e) { responseDiv.textContent = `Erro: ${response.status}. O proxy não respondeu corretamente.`; }
                    button.disabled = false; button.textContent = "Resolver com IA 🤖";
                }
                else { handleAISuccess(response.responseText, responseDiv, button, dataset); }
            },
            onerror: (error) => {
                responseDiv.textContent = 'Erro de rede. Não foi possível conectar ao proxy.';
                button.disabled = false; button.textContent = "Resolver com IA 🤖";
    ...
    // ##################################################################
    // #           MODIFICAÇÃO 4: Criar função applyHighlight             #
    // ##################################################################
    /**
     * Aplica apenas o destaque (borda azul) e rola até o item.
     */
    ...
    // ##################################################################
    // #        MODIFICAÇÃO 5: Atualizar listener do Foco (🎯)            #
    // ##################################################################
    window.addEventListener('ActiveQuestionChanged', (event) => {
        ...
        if (document.getElementById('qia-focus-toggle')?.checked) {
            ...
            applyHighlight(newText); // Usa a nova função
        }
    });
    // ##################################################################


    // --- FUNÇÕES DE AUTO-PREENCHIMENTO (v13.24) ---
            ...
            if (kind === 'MATCH') {
                ...
                for (const pair of pairs) {
                 ...
                 for (const item of pairs) {
                    ...
                    targetEl.click(); // 1. Clica na lacuna

                    let keyToPress;
                    try {
                        await waitForElement('.keyboard-interaction-shortcuts', 2000); // 2. Espera o pop-up
                        await new Promise(resolve => setTimeout(resolve, 100)); // 3. Espera extra
                        keyToPress = findDragDropKeyByText(item.text); // 4. Encontra a tecla
                    } catch (e) {
                    ...
                    simulateKeyPress(keyToPress); // 5. Simula a tecla
                    successCount++;
                    await new Promise(resolve => setTimeout(resolve, 600));
                }
            }
    ...
    // ##################################################################
    // #           MODIFICAÇÃO 6: Reescrever Loop do PA (🚀) (v13.28)     #
    // ##################################################################
    /**
     * Inicia o loop principal que controla o Piloto Automático.
     */
    function startMasterLoop() {
        if (masterInterval) clearInterval(masterInterval);

        // Roda o loop a cada 750ms
        masterInterval = setInterval(async () => {
            if (isAutoTaskRunning) return; // Se uma tarefa já está rodando, pula este ciclo

            isAutoTaskRunning = true; // Trava
            try {
                await masterAutoPilotTask();
            } catch (e) {
                console.error("Erro no Piloto Automático:", e);
            }
            isAutoTaskRunning = false; // Libera
        }, 750); // Intervalo de 750ms
    }

    /**
     * A tarefa principal do Piloto Automático, executada a cada 750ms.
     */
    async function masterAutoPilotTask() {
        ...
        if (activeQuestionEl) {
            ...
            // Se não há pergunta, reseta o texto
            currentActiveQuestionText = "";
        }

        // ATUALIZA A GUI (Foco ou Destaque) MESMO SE O TEXTO FOR O MESMO
        // Isso garante que se o usuário ligar o 🎯 no meio, ele funcione.
        if (document.getElementById('qia-focus-toggle')?.checked) {
            applyFocus(currentActiveQuestionText);
        } else {
        ...
        // 3. Lógica de Ação
        if (activeItem) {
            // Prioridade 1: Clicar em "Auto-Preencher" (➡️) se estiver disponível
            ...
            if (autoFillBtn) {
                ...
                autoFillBtn.click(); // A função de clique já marca como 'processed'
                await new Promise(r => setTimeout(r, 1000)); // Espera o preenchimento
                return; // Próximo ciclo
            }

            // Prioridade 2: Clicar em "Resolver com IA" (🤖) se for uma pergunta nova
            const solveBtn = activeItem.querySelector('.qia-ai-button:not([data-processed="true"])');
            if (solveBtn) {
                console.log("PA: Clicando em 'Resolver com IA'...");
                solveBtn.click(); // A função de clique já marca como 'processed'
                return; // Próximo ciclo
            }
        }
        ...
        if (submitBtn && !submitBtn.disabled) {
            ...
            submitBtn.click();
            await new Promise(r => setTimeout(r, 1000)); // Espera a transição
            return;
        }
